...
function getCourseFile(courseList) {
    for (let index = 0; index < courseList.length; index++) {
        ...
        if (course.Fee <= 50) {
            console.log(`${course.CourseId} has a fee of $${course.Fee}, less than $50.`);
        }
        if (course.Location == "Classroom 1") {
            console.log(`${course.CourseId} meets in ${course.Location}`);
        }
    }
}

getCourseFile(courses);
